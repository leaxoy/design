...
/**
 * 使用Redis存储Token Created by lxh on 4/20/16.
 */
@Component
public class RedisTokenManager implements TokenManager {
  ...
  public AuthToken createToken(String accountName) {
    ...
    //存储到redis并设置过期时间
    redis.boundValueOps(accountName).set(token, TokenConstant.TOKEN_EXPIRES_HOUR,
            TimeUnit.HOURS);
    return model;
  }
  ...
  public boolean checkToken(AuthToken model) {
    ...
    redis.boundValueOps(model.getAccountName()).expire(TokenConstant.TOKEN_EXPIRES_HOUR,
            TimeUnit.HOURS);
    return true;
  }
